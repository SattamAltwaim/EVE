...
Runs two scenarios back-to-back on identical hardware:
  ...
  A) ISOLATED  — EVE phases run from a clean allocator (no prior backward).
                 Matches what Experiment 5 in the notebook measures (~86ms).
...
# Force a fresh import so we pick up whatever eve.py is actually on disk
...
PHASE_NAMES = [
    "P_str  strength signal",
    "P2a    moments + global_max_sqrt_v",
    "P2b    offspring construction",
    "P_save param backup clone",
    "P4a    L_base forward",
    "P4b    K in-place probe cycles",
    "P5     fitness + softmax",
    "P6     weighted einsum update",
    "P7     temperature adapt",
]
...
def timed_eve_step(
    ...
    opt._update_strength_signal(current_loss)
    cuda_sync(); t_str = time.perf_counter()
    ...
    # ── P2b: offspring construction ───────────────────────────────────────────
    offspring_map: Dict[int, torch.Tensor] = {}

    cuda_sync(); t2b_s = time.perf_counter()
    ...
    # ── P_save: parameter backup ──────────────────────────────────────────────
    cuda_sync(); t_save_s = time.perf_counter()
    saved: Dict[int, torch.Tensor] = {
        p.data_ptr(): p.data.clone() for _, p in params_with_grad
    }
    cuda_sync(); t_save = time.perf_counter()
    timing["P_save param backup clone"].append((t_save - t_save_s) * 1e3)
    mem_log["after_P_save"].append(mem_mb())

    # ── P4a: L_base forward ───────────────────────────────────────────────────
    was_training = model.training
    model.eval()

    cuda_sync(); t4a_s = time.perf_counter()
    with torch.no_grad():
        L_base = loss_fn(model(inp), tgt)
    cuda_sync(); t4a = time.perf_counter()
    timing["P4a    L_base forward"].append((t4a - t4a_s) * 1e3)
    mem_log["after_P4a"].append(mem_mb())

    # ── P4b: K in-place probe cycles ──────────────────────────────────────────
    probe_losses: List[torch.Tensor] = []

    cuda_sync(); t4b_s = time.perf_counter()
    with torch.no_grad():
        for k in range(K):
            for group, p in params_with_grad:
                p.data.add_(offspring_map[p.data_ptr()][k], alpha=group["lr"])
    ...
    cuda_sync(); t4b = time.perf_counter()
    timing["P4b    K in-place probe cycles"].append((t4b - t4b_s) * 1e3)
    mem_log["after_P4b"].append(mem_mb())
    ...
    timing["P5     fitness + softmax"].append((t5 - t5_s) * 1e3)
    mem_log["after_P5"].append(mem_mb())

    # ── P6: weighted einsum update ────────────────────────────────────────────
    cuda_sync(); t6_s = time.perf_counter()
    for group, p in params_with_grad:
    ...
    cuda_sync(); t6 = time.perf_counter()
    timing["P6     weighted einsum update"].append((t6 - t6_s) * 1e3)
    mem_log["after_P6"].append(mem_mb())

    # ── P7: temperature ───────────────────────────────────────────────────────
    cuda_sync(); t7_s = time.perf_counter()
    ...
    opt.beta_sel *= math.exp(alpha_beta * (H_t - rho * math.log(K)))
    opt.beta_sel  = max(beta_min, min(beta_max, opt.beta_sel))
    opt._prev_loss = current_loss
    cuda_sync(); t7 = time.perf_counter()
    timing["P7     temperature adapt"].append((t7 - t7_s) * 1e3)
    mem_log["after_P7"].append(mem_mb())
...
def print_timing_table(
    ...
    print(f"  {label}")
    print(f"{'─'*72}")
    print(f"  {'Phase':<40s} {'Mean ms':>8s} {'p50':>8s} {'p95':>8s} {'%':>6s}  {'Alloc MB':>9s}")
    print(f"  {'─'*40} {'─'*8} {'─'*8} {'─'*8} {'─'*6}  {'─'*9}")
    ...
    for i, (phase, vals) in enumerate(timing.items()):
        ...
        print(f"  {phase:<40s} {a.mean():8.2f} {np.median(a):8.2f} "
              f"{np.percentile(a,95):8.2f} {100*a.mean()/total:5.1f}%  "
              f"{alloc_mb:8.1f}")

    print(f"  {'─'*40} {'─'*8}")
    print(f"  {'TOTAL EVE phases':<40s} {total:8.2f}")
...
def run_isolated(loader, loss_fn, n_warmup=5, n_measure=20):
    ...
    for step_idx in range(n_warmup + n_measure):
        ...
        # Isolated: compute gradients via backward (needed for state),
        # but the allocator is not fragmented by large activation buffers
        # being freed just before EVE runs.
        model.train()
        opt.zero_grad()
        ...
        if step_idx < n_warmup:
            # warmup: call real step to build optimizer state
            opt.step(model=model, loss_fn=loss_fn, data=(xb, yb),
                     current_loss=current_loss)
            continue

        # Clear CUDA cache between steps to simulate clean-allocator scenario
...
def run_real(loader, loss_fn, n_warmup=5, n_measure=20):
    ...
    for step_idx in range(n_warmup + n_measure):
        ...
        # ── Standard forward + backward (activation memory NOT freed yet) ──
        model.train()
        opt.zero_grad()
        loss = loss_fn(model(xb), yb)
        loss.backward()
        current_loss = loss.item()
        # NOTE: after .backward(), activation tensors are freed by autograd
        # but CUDA allocator may not have returned the blocks to the pool yet,
        # leaving fragmented gaps that make large allocations inside EVE slow.
...
def print_comparison(iso_timing, real_timing, real_bwt):
    ...
    print("="*72)
    print(f"  {'Phase':<40s} {'Isolated ms':>12s}  {'Real ms':>10s}  {'Slowdown':>9s}")
    print(f"  {'─'*40} {'─'*12}  {'─'*10}  {'─'*9}")
    ...
    for phase in PHASE_NAMES:
        ...
        print(f"  {phase:<40s} {iso_ms:12.2f}  {real_ms:10.2f}  {sx:8.2f}x")
    print(f"  {'─'*40} {'─'*12}  {'─'*10}  {'─'*9}")
    print(f"  {'TOTAL EVE phases':<40s} {iso_total:12.2f}  {real_total:10.2f}  "
