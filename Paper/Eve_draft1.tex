...
\hypersetup{
  ...
  citecolor=accent,
  urlcolor=accent2,
  pdftitle={EVE: Evolutionary Virtual Exploration},
...
\fancyhead[R]{\small\color{graytext}Draft}
...
\begin{center}
  ...
  Selection with Probe-Based Fitness Evaluation}\\[14pt]
  {\normalsize\color{graytext}\itshape Draft \textemdash\
  March 2026}\\[6pt]
\end{center}
...
We present \textbf{EVE}---Evolutionary Virtual Exploration---an optimizer
...
principled exploration--exploitation control via adaptive selection
...
EVE addresses all four by introducing a minimal evolutionary layer:
...
update that is provably at least as good as any single strategy
(in expectation over the selection), and the brood size $K$ provides
...
EVE provides a softer form of sharpness awareness: offspring directions
...
dominate, the probe overhead may be 10--20\% in practice.
}
...
temperature adaptation rate $\alpha_\beta$ have robust defaults that
should not need task-specific tuning.
...
is near-optimal at every step.
The probe will consistently assign $w_1 \to 1$ via soft selection, and
EVE converges to Adam's trajectory.
...
EVE's offspring provide this: the complementary offspring amplifies
...
The selection mechanism determines the optimal blend automatically
at each step.
...
(indicating the minimum is sharp but not deep), the contrarian receives
...
At suboptimal learning rates, different offspring respond differently:
\begin{itemize}[leftmargin=1.6em, itemsep=3pt]
  \item $\eta$ too large: The gradient offspring $\mathbf{d}_2$
    ...
    Selection shifts toward $\mathbf{d}_1$.
  \item $\eta$ too small: The contrarian offspring $\mathbf{d}_4$
    ...
    Selection shifts toward $\mathbf{d}_4$.
  \item $\eta$ correct: The Adam offspring $\mathbf{d}_1$ performs
    well, but the complementary offspring $\mathbf{d}_3$ recovers
    any dimensions where Adam is nonetheless stuck.
    Selection balances all four.
\end{itemize}

This implicit adaptation widens the basin of acceptable learning rates:
EVE at a mediocre $\eta$ should approach the performance of Adam at
a tuned $\eta$.
...
This maintains EVE's constant overhead across worker counts.
...
We have presented EVE, an optimizer that embeds evolutionary selection
...
enabling complementary offspring to rescue dimensions where Adam's
...
and the selection is honest.
The probe ensures honesty (real loss on real data), the offspring
ensure diversity (by construction), and the softmax ensures softness
(no strategy is ever fully discarded).

Where Adam tells you the single best guess, EVE asks: what if we
tried $K$ different ideas and kept the best?
The answer, it turns out, is worth about one-third of a forward pass.
