...
The fitness values are transformed into a probability distribution over the momentum spectrum using temperature-scaled softmax:

\begin{equation}
  w_k = \frac{\exp(\beta_{\mathrm{sel}}\mathcal{F}_k)}{\sum_{j=1}^{K} \exp(\beta_{\mathrm{sel}}\mathcal{F}_j)}
\end{equation}
...
\begin{tabular}{@{}llp{8cm}@{}}
  ...
  $\beta_{\mathrm{sel}}$ & $1.0$ & Selection temperature for the softmax distribution. Higher values aggressively exploit the fittest offspring; lower values blend them more evenly. \\
  \bottomrule
\end{tabular}
\end{center}
